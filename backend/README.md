...
EMAIL_USE_SSL=0
SCHEDULE_ENABLED=0
SCHEDULE_DAILY_ENABLED=1
SCHEDULE_DAILY_TIME=11:00
SCHEDULE_WEEKLY_ENABLED=1
SCHEDULE_WEEKLY_DAY=5
SCHEDULE_WEEKLY_TIME=11:00
SCHEDULE_SEND_BITRIX=1
SCHEDULE_SEND_EMAIL=0
WEB_UI_HOST=0.0.0.0
WEB_UI_PORT=8080
WEB_UI_USER=admin
WEB_UI_PASSWORD=
...
Создать один файл отчётов и отправить его по email:

```bash
cd backend/src
../venv/bin/python generate_reports.py --mode all --send-email
```
...
- опциональная отправка файла по email;
- ввод и сохранение секретов/подключений на странице `/settings`;
- настройка автоматической генерации daily/weekly и каналов рассылки;
- список последних `.xlsx`;
- просмотр созданной книги в браузере по листам через `/view`;
- автоматическое открытие созданной книги в окне просмотра после генерации;
- кнопки навигации по листам и меню действий в окне просмотра;
- управление пользователями на странице `/users`, доступное только роли
  `admin`;
- скачивание сформированного файла.

Страница настроек не показывает сохранённые секреты в HTML: для паролей и
webhook отображается только статус `задано` / `не задано`. Пустое секретное
поле при сохранении оставляет старое значение, для удаления есть отдельный
чекбокс `Очистить значение`.

## Пользователи web-интерфейса

Первый вход выполняется пользователем из `WEB_UI_USER` и `WEB_UI_PASSWORD`.
После создания или изменения пользователей web-интерфейс хранит учётные записи
в `backend/users.json`; файл содержит только хэши паролей и не попадает в git.

Страница `/users` доступна только пользователям с ролью `admin`. На ней можно:

- создать нового пользователя;
- выдать роль `admin`;
- изменить пароль существующего пользователя;
- заблокировать или разблокировать доступ;
- удалить пользователя.

Интерфейс не даёт удалить или заблокировать текущего пользователя, а также
последнего активного `admin`.

## Одновременная работа пользователей

Web-сервер использует `ThreadingHTTPServer` и может принимать несколько
запросов одновременно. Общие ресурсы защищены блокировками внутри процесса:

- сохранение `backend/.env` и обновление runtime-настроек;
- чтение/изменение `backend/users.json`;
- генерация Excel-книги и последующая отправка в Bitrix24/email.

Итоговая книга сначала сохраняется во временный файл в `backend/output/`, затем
атомарно заменяет целевой `.xlsx`. Поэтому просмотр или скачивание не должны
попасть на частично записанный файл.
...
Email-рассылка использует SMTP и отправляет сформированный `.xlsx` вложением.
Нужны параметры:

- `EMAIL_SMTP_HOST`, `EMAIL_SMTP_PORT` — SMTP-сервер и порт;
- `EMAIL_SMTP_LOGIN`, `EMAIL_SMTP_PASSWORD` — логин и пароль SMTP, если сервер
  требует авторизацию;
- `EMAIL_FROM` — адрес отправителя;
- `EMAIL_TO` — один или несколько получателей через запятую или `;`;
- `EMAIL_USE_TLS=1` — STARTTLS, обычно для порта `587`;
- `EMAIL_USE_SSL=1` — SMTP over SSL, обычно для порта `465`.

## Расписание

Автоматическая генерация настраивается на странице `/settings` и хранится в
переменных `SCHEDULE_*`. При сохранении настроек web-интерфейс обновляет
crontab текущего пользователя, удаляя старые строки проекта по маркеру
`1c_report generate_reports`.

Основные параметры:

- `SCHEDULE_ENABLED` — включает/выключает cron-задачи проекта;
- `SCHEDULE_DAILY_ENABLED`, `SCHEDULE_DAILY_TIME` — дневной отчёт;
- `SCHEDULE_WEEKLY_ENABLED`, `SCHEDULE_WEEKLY_DAY`, `SCHEDULE_WEEKLY_TIME` —
  недельный отчёт;
- `SCHEDULE_SEND_BITRIX`, `SCHEDULE_SEND_EMAIL` — каналы автоматической
  рассылки после генерации.

Для установки расписания из консоли:
...
backend/
  ...
  src/
    ...
    scheduler.py       установка cron-расписания
